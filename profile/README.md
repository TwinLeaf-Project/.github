<div align="center">

# 🌱 TwinLeaf Project

**一個香港人的開源項目**  
*A Hong Kong open-source project for nicamoq, Hong Kong, and all — We Are AI PC Ready.*

[![網站](https://img.shields.io/badge/網站-入口網站-1E3A8A?style=flat-square)](https://sites.google.com/view/twinleaf-os-hk/)
[![Facebook](https://img.shields.io/badge/Facebook-TwinLeaf-1877F2?style=flat-square)](https://facebook.com/TwinLeaf)
[![社群](https://img.shields.io/badge/社群-Stoat.chat-8A2BE2?style=flat-square)](https://stt.gg/cqb4D0fD)

</div>

---

## 🎯 我們的使命

我們相信，科技應回歸人性——**尊重、美感、易用、自由**。

TwinLeaf Project 是一個源於香港的開源項目，致力於為香港用戶打造尊重私隱、易於使用、美觀自由的數位工具。

核心理念：

| 理念 | 說明 |
|------|------|
| 🤝 **尊重** | 根源切除私隱監控，不收集任何個人資料 |
| 🎨 **美感** | 簡約優雅的介面設計，賞心悅目 |
| ✨ **易用** | 繁體中文開箱即用，降低使用門檻 |
| 🕊️ **自由** | 你的電腦，只屬於你 |

---

## 📦 主要項目

| 項目 | 說明 | 狀態 |
|------|------|------|
| [**TwinLeaf OS**](https://github.com/TwinLeaf-Project/twinleaf-os) | 基於 Debian + KDE 的 Linux 發行版 | 🚧 開發中 |
| **TwinLeaf Browser** | 私隱優先的網頁瀏覽器 | 📋 規劃中 |
| **TwinLeaf Assistant** | 本地化 AI 助理 | 🔮 籌備中 |
| **TwinLeaf Optimizer** | 系統優化工具 | 📋 規劃中 |

---

## 🖥️ 硬體定位

TwinLeaf OS 專為以下設備設計：

| 類型 | 代表機型 | 狀態 |
|------|----------|------|
| **AI PC (2024+)** | Lenovo IdeaPad Slim 3 (R7 8840HS) | ✅ 主力開發平台 |
| **舊筆記本 (2019+)** | Lenovo Yoga (i5-8250U) | ✅ 相容性測試平台 |
| **桌面主機** | 任何 AMD/Intel PC | ✅ 可用 |

---

## 🤝 參與我們

TwinLeaf Project 由博藝工作室（Benmak Workshop）一人獨立開發，但開源社群就是最好的夥伴。

| 方式 | 說明 |
|------|------|
| **🐛 問題回報** | [GitHub Issues](https://github.com/TwinLeaf-Project/twinleaf-os/issues) |
| **💬 社群討論** | [Stoat.chat](https://stoat.chat/twinleaf) |
| **📖 文件閱讀** | [GitHub Wiki](https://github.com/TwinLeaf-Project/twinleaf-os/wiki) |
| **🔧 貢獻代碼** | Fork → 修改 → Pull Request |

---

## 📜 許可證

| 組件 | 許可證 |
|------|--------|
| 原創腳本 | GPL-3.0 |
| 文檔 | CC BY-SA 4.0 |
| 品牌資產 | CC BY-NC-ND 4.0 |

---

## 🌱 關於博藝工作室

TwinLeaf Project 由香港 **博藝工作室（Benmak Workshop）** 創立。靈感來自日本藝人 **守崎風花（nicamoq）** 從普通網民到成熟表演者的轉變，象徵「勇於嘗試」與「溫柔平衡」。


**以尊重為本，讓自由與美感同行。**

---

<div align="center">

**🌱 願每一次啟航，都帶著溫柔的勇氣。**

[GitHub](https://github.com/TwinLeaf-Project) · [Facebook](https://facebook.com/TwinLeaf) · [Stoat](https://stt.gg/cqb4D0fD)

</div>
